...
if __name__ == "__main__":
    ...
    def _run_setup_wizard():
        ...
        print()
        print("  =============================================")
        print("  Welcome to BrainBox!")
        print("  =============================================")
        print()
        print("  Let's get you set up. You need 3 API keys.")
        print("  (Google and Pinecone are free, Anthropic is ~$0.002/question)")
        print()

        keys = {}
        prompts = [
            ("ANTHROPIC_API_KEY", "Anthropic (generates answers)",
             "https://console.anthropic.com/settings/keys",
             "Starts with sk-ant-"),
            ("GOOGLE_API_KEY", "Google AI (embeddings + search + audio)",
             "https://aistudio.google.com/app/apikey",
             "Starts with AIza"),
            ("PINECONE_API_KEY", "Pinecone (stores your documents)",
             "https://app.pinecone.io",
             "Starts with pcsk_"),
        ]

        for key_name, desc, url, hint in prompts:
            print(f"  Step: {desc}")
            print(f"  Get your key here: {url}")
            print(f"  ({hint})")
            while True:
                value = input("  Paste key: ").strip()
                if not value:
                    print("  Key can't be empty, try again.")
                    continue
                if "xxxxx" in value:
                    print("  That looks like a placeholder, paste your real key.")
                    continue
                keys[key_name] = value
                print("  Saved!\n")
                break
        ...
        with open(env_path, "w") as f:
            f.write("# BrainBox Configuration (auto-generated by setup wizard)\n\n")
            for k, v in keys.items():
                f.write(f"{k}={v}\n")
            f.write("\nPINECONE_INDEX_NAME=brainbox\n")
            f.write("CLAUDE_MODEL=claude-haiku-4-5-20251001\n")
            f.write("CHUNK_SIZE=500\n")
            f.write("CHUNK_OVERLAP=50\n")

        print("  =============================================")
        print("  Setup complete! Starting BrainBox...")
        print("  =============================================\n")
    ...
    if not os.path.exists(env_path):
        ...
        try:
            ...
            ans = input("  Run setup wizard? (y/n): ").strip().lower()
            if ans in ("y", "yes", ""):
                _run_setup_wizard()
            else:
                print("  Edit .env manually and try again.\n")
                exit(1)
